...
FluentAuth 기반 기능은 **유사 기존 구현을 먼저 전체 읽고 분석**한다.  
분석 없이 설계하면 훅 우선순위 충돌, 패리티 누락, 불필요한 후공정이 발생한다.  
상세 교훈 및 SOP: `RETROSPECTIVE.md`
...
> ✅ = Docker exec 검증 완료 | ⬜ = 카카오 앱 연동 후 수동 테스트 필요

**정적 검증 (완료)**
- ✅ PHP 문법 검사 (`php -l`)
- ✅ 플러그인 로드/활성화 (`wp plugin status`)
- ✅ FluentAuth 없을 때 관리자 알림 표시, 버튼 미노출
- ✅ 카카오 로그인 버튼 wp-login.php 표시 확인
- ✅ TwoFaHandler 클래스 존재 및 isEnabled 로직 확인

**기본 플로우 (수동 테스트 필요)**
- ⬜ 신규 사용자: WP 계정 자동 생성 + FluentCRM 등록 확인
- ⬜ 기존 사용자: 동일 이메일 계정 자동 연결 확인
- ⬜ 기존 사용자: `fak_kakao_id` 메타로 연결 (이메일 변경 후에도 유지)
...
- ⬜ state 재사용 (두 번째 요청 실패)
- ⬜ 쿠키 없는 콜백 → FluentAuth에 위임 (카카오 핸들러 개입 없음)
...
**예외 처리 (수동 테스트 필요)**
- ⬜ 로그인된 상태에서 다른 카카오 계정 로그인 시도 → `email_mismatch` 오류
- ⬜ 신규가입 비활성화 + 신규 카카오 계정 → `signup_disabled` 오류
